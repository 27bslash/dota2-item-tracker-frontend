...
const itemSearch = (
  ...
  const allItems = data
    .map((match) => match["items"].map((item) => item.key))
    .flat();
  data.forEach((match) => {
    ...
    for (const item of match["items"]) {
      ...
      if (
        ...
        if (dict[`${symbol}${name}`]) {
          ...
          dict[`${symbol}${name}`] = { matches: [match], index: i };
        }
    ...
    const neutral =
      searchRes["names"].has(match["item_neutral"]) ||
      searchRes["names"].has(`item_${match["item_neutral"]}`);
    if (neutral) {
      matches.push(match);
      if (dict[match["item_neutral"]]) {
        dict[match["item_neutral"]]["matches"].push(match);
      } else {
        dict[match["item_neutral"]] = { matches: [match], index: i };
      }
  ...
  if (symbol === "-") {
    for (const item of searchRes["names"]) {
      const filteredMatches = data.filter((match) =>
        match["items"]
          .map((x) => x.key)
          .every((x: string) => allItems.includes(item) && item !== x)
      );
