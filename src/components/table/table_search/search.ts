  ...
  players.forEach((player) => {
    const data = matchData.filter((match) => {
      try {
        ...
        console.error("Error in player search:", e);
        return false;
      }
    });
    if (data.length) {
      const totalFilteredMatches = matchData.filter(
        (match) => match["name"] === player
      );
      dict[`${symbol}${player}`] = {
        matches: data,
        index: i,
...
const roleSearch = (matchData: DotaMatch[], searchValue: string, i = 0) => {
  ...
  const partialMatches = roles.filter((x) =>
    x.toLowerCase().includes(searchValue.toLowerCase())
  );
  for (const partialmatch of partialMatches) {
    const data = matchData.filter(
      (match) => partialmatch.toLowerCase() === String(match.role).toLowerCase()
    );
    if (data.length) {
...
const search = (
  ...
  searchTerms.forEach((term: string, i: number) => {
    if (itemData) {
      const itemSearchResults = itemSearch(
        term,
        matchData,
        itemData,
        undefined,
    ...
    if (herolist && heroName) {
      const draftSearchResults = new DraftSearch().handleDraftSearch(
        matchData,
        herolist,
        term,
        heroName,
