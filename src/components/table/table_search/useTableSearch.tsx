...
export const useTableSearch = (searchTerms: string[], totalMatchData: DotaMatch[], heroList: Hero[], playerList: string[] | undefined, items: Items, role: string | undefined, heroName: string) => {
    const data = role ? totalMatchData.filter((match) => match.role === role) : totalMatchData
    const minLengthResults = searchTerms.some((term) => term.length >= 2)
    if (!minLengthResults) {
        return { matches: [], searchResults: {} }
    }
    const searchResults = search(searchTerms, data, items!, heroList, playerList, heroName)
    ...
    const matches = [...totalMatchData].filter((x) => (matchIds.includes(x.id) && x.role === role) || (matchIds.includes(x.id) && !role))
    return { matches, searchResults }
}