        ...
        const async_get = async () => {
            if (type === 'hero') {
                ...
                for (const colorObj of json['colors']) {
                    ...
                    setHeroColor(
                        `rgb(${colorObj['color'][0]}, ${colorObj['color'][1]}, ${colorObj['color'][2]})`
                    )
                    const colorSUm =
                        colorObj['uncontrasted'][1] +
                        colorObj['uncontrasted'][2]
                    const greenRatio = colorObj['uncontrasted'][1] / colorSUm
                    if (
                        (Math.max(...colorObj['uncontrasted']) ===
                            colorObj['uncontrasted'][1] ||
                            Math.max(...colorObj['uncontrasted']) - 50 <=
                                colorObj['uncontrasted'][1]) &&
                        colorObj['uncontrasted'][1] -
                            colorObj['uncontrasted'][2] >
                            50 &&
                        (greenRatio > 0.6 || colorObj['uncontrasted'][1] > 170)
                    ) {
                        continue
                    }
                    generateColorPalette(
                        [
                            colorObj['uncontrasted'][0],
                            colorObj['uncontrasted'][1],
                            colorObj['uncontrasted'][2],
                        ],
                        targetHero
