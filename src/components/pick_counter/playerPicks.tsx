...
export const PlayerPicks = ({ matchKey }: PlayerPickProps) => {
    const heroCount: Record<
        string,
        Record<string, { picks: number; win: number }>
    > = {}
    ...
    for (const match of matchData) {
        if (
            heroCount[match[matchKey]] &&
            heroCount[match[matchKey]][match['role']]
        ) {
            ...
            if (heroCount[match[matchKey]]) {
                ...
                heroCount[match[matchKey]] = {
                    [match['role']]: { picks: 1, win: match['win'] },
                }
            }
        }
    }
    ...
    let sortedData = Object.entries(heroCount).sort(
        ([, aValue], [, bValue]) => {
                        ...
                        >
                            {matchKey === 'hero' ? (
                                <DraftImage
                                    ...
                                    highlight={false}
                                    onClick={() =>
                                        updateData(cleanName, 'hero', 'hero')
                                    }
                                ></DraftImage>
                            ) : (
                                <Typography
