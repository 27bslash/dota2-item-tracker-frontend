...
export const countItems = (data: NonProDataType[], itemData: Items) => {
    ...
    seenItems.forEach((x) => {
        ...
        if (filteredItemTimes) {
            ...
            const avgTime =
                filteredItemTimes.reduce((a, b) => a + b) /
                filteredItemTimes.length
            const time = Math.min(medianTime, avgTime)
            if (
                !key.match(/__\d+/g) ||
                (key.match(/__\d+/g) && avgTime <= 800)
            ) {
                itemValues[key] = {
        ...
        .map((item) => {
            ...
            const o: RawItemBuild = [
                item[0],
                {
                    ...
                    adjustedValue:
                        (filteredData.length / (filteredData.length + count)) *
                        100,
                    time: item[1]['time'],
                },
            ]
