...
const NavSearch = ({
  ...
  useEffect(() => {
    if (filterHeroes) {
      if (
        (value.length > 1 && !!sortedPlayers.length) ||
        (sortedHeroes.length !== heroList.length && !filteredByButton?.length)
      ) {
      ...
      setShowSearchResults(
        !!sortedPlayers.length || sortedHeroes.length !== heroList.length
      );
    }
  }, [sortedHeroes]);
  useEffect(() => {
  ...
  return (
    <Box className="nav-search" bgcolor={theme.palette.primary.main}>
      <TextField
        sx={{
          width: "100%",
