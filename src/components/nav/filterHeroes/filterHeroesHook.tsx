...
export const useFilterHeroes = (heroList: Hero[], filteredHeroes: string[] | undefined, value: string) => {
    ...
    useEffect(() => {
        ...
        if (value.length > 1) {
            ...
            const copy = [...heroList]
            const heroMatches = matchSorter(copy.map((x) => {
                x.name = x.name.replace(/-|_/g, ' ')
                return x
            }), searchText, { keys: [{ threshold: matchSorter.rankings.CONTAINS, key: 'name' }] }).slice(0, 8)
            ...
            const legacyHeroMatches = matchSorter(legacyList.map((x) => {
                x.name = x.name.replace(/-|_/g, ' ')
                return x
            }), searchText, { keys: [{ threshold: matchSorter.rankings.CONTAINS, key: 'name' }] }).slice(0, 8)
