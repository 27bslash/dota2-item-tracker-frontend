...
function HeroCard({ stats, heroName, role, searching, idx, highlight, sortByTrend }: HeroCardProps) {
            ...
            {!searching && !!stats
                && (
                    <div className="win-stats" style={{ width, maxWidth: width }}>
                        <Typography variant='body2' fontSize={14} className="picks">{stats.picks}
                            {stats.trend >= 0 ? (
                                <span className='trend' onClick={() => sortByTrend()} style={{ color: 'green' }}>+{stats.trend}</span>
                            ) : <span className='trend' onClick={() => sortByTrend()} style={{ color: 'red' }}>{stats.trend}</span>
                            }
                        </Typography>
                        <Typography variant='body2' fontSize={14} className="wins">{stats.wins}</Typography>
                        <Typography variant='body2' fontSize={14} className="winrate" style={{ color: colourWins(stats.winrate) }}>
                            {stats.winrate.toFixed(0)}
                            %
                        </Typography>
                        <Typography variant='body2' fontSize={14} className="bans">{stats.bans}</Typography>
                    </div>
                )}
        </div>
    )
