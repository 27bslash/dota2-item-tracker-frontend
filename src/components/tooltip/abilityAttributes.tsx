...
const AbilityAttributes = (props: any) => {
    const dmgType = damageType(props.ability.damage)
    const pierce = props.ability.immunity === 3 || props.ability.immunity === 1 ? "yes" : "no";
    const dispel = dispellable(props.ability.dispellable)
    return (
        <div className="attributes" style={{ borderBottom: 'solid 1px gray', paddingBottom: '5px' }}>
            {dmgType &&
                <AbilityAttribute heading='DAMAGE TYPE:' color={dmgType[1]} value={dmgType[0]} />
            }
