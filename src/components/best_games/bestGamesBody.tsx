        ...
        return (
          ...
          >
            <td className="benchmark-cell">
              <a className="benchmark-icon" href={`/player/${match["name"]}`}>
                {match["name"].replace(/\(smurf.*\)/, "")}
              </a>
            </td>
            ...
            <td className="benchmark-cell" style={{ padding: 0 }}>
              <FontAwesomeIcon
                ...
                icon={faCopy}
                color="white"
                onClick={() => navigator.clipboard.writeText(String(match.id))}
              />
            </td>
            <td
            ...
            >
              <div
                className="benchmark-icon svg-icon"
                id={match.role ? match.role.replace(" ", "-") : ""}
                onClick={() => props.updateRole(match.role)}
              ></div>
            </td>
            {benchmarkKeys.map((k: BenchMarksKeys, idx: number) => {
              if (k in match.benchmarks) {
                ...
                return (
                  ...
                  >
                    <>
                      <span className="benchmark-pct" style={{ color: color }}>
                        {cleanDecimal(pct)}%{" "}
                      </span>
