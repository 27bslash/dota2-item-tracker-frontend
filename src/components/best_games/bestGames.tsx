...
const BestGames = ({ updateRole, updatePageNumber }: BestGamesProps) => {
    ...
    const getBenchMarksFromData = () => {
        ...
        const cleanVal = (
            match: DotaMatch,
            key: keyof DotaMatch | BenchMarksKeys
        ) => {
            if (key in match) {
                return (
                    +cleanDecimal(match[key as keyof DotaMatch] as number) /
                    (['gpm', 'xpm', 'tower_damage', 'last_hits_ten'].includes(
                        key
                    )
                        ? 1
                ...
                return match['benchmarks'][bKey]
                    ? +cleanDecimal(+match['benchmarks'][bKey]['raw'])
                    : 1
            }
            return 0
        }
        const calculateMaxValues = (key: string) =>
            filteredData.reduce((max, match) => {
                const value = cleanVal(match, key as keyof DotaMatch)
                return Math.max(max, value)
            }, 0)

        const maxValues = benchmarkKeyArr.map(calculateMaxValues)
        const matchBenchmarkKeys = [
            'gold_per_min',
            'xp_per_min',
            'kills_per_min',
            'last_hits_per_min',
    ...
    const sumBenchmarks = () => {
        ...
        if (sortKey) {
            const sortedByKey = [...filteredData].sort(
                (a, b) =>
                    +b['benchmarks'][sortKey]['raw'] -
                    +a['benchmarks'][sortKey]['raw']
            )
            if (sortDirection === 'desc') {
                sortedByKey.reverse()
            }
            // console.log(s);
            sorted = sortedByKey.map(
                (match) =>
                    bmarks.find((m) => m[0] === match['id']) as [number, number]
            )
        } else {
        ...
        const filtered = filteredData.filter((x) =>
            sorted
                .slice(0, 2)
                .map((x) => x[0])
                .includes(x['id'])
        ...
        if (filtered.length) {
            ...
            const sortedBenchmarks = Object.keys(
                filtered[0]['benchmarks']
            ).sort((a, b) => {
